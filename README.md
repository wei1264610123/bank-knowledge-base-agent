...
1. **登录系统**：使用管理员账号登录
2. **知识库管理**：进入"知识库管理"页面
   - 上传文档（支持PDF、Word、Excel、PPT、TXT、Markdown）
   - 查看文档处理状态
   - 管理文档分类
   - 查看文档分块详情
3. **用户管理**：进入"用户管理"页面
   - 查看系统统计（数据面板：今日/本周/累计提问、好评差评、热门问题、待补知识 TOP）
   - 管理用户状态、重置密码
4. **报表导出**：数据面板可按时间范围一键导出 CSV 报表（问答明细 / 审计日志 / 反馈记录），用 Excel 直接打开，方便银行留档
5. **回答质量抽查**：用户管理 →"🔍 质量抽查"Tab，抽查 AI 回答质量（未评判的排前面，可筛选已评判/未评判），查看用户反馈并给出 1-5 分评判，配合反馈做质量管控
6. **待补知识**：查看用户"没找到答案"时提交的问题，处理后补充到知识库（知识闭环）
7. **审计日志**：查看登录、上传、删除、反馈、质量评判等敏感操作留痕（银行合规）

### 普通用户功能

1. **注册/登录**：注册新账号或使用已有账号登录
2. **智能问答**：进入"智能问答"页面
   - 输入问题进行知识库问答（首页展示智能推荐问题，支持快捷追问）
   - 查看回答的引用来源（引用片段按提问关键词高亮，点击可预览文档原文并定位）
   - 管理对话历史（会话自动命名，支持重命名/删除）
   - 📋 复制回答、🔄 重新生成（AI 已评价的回答不可重新生成）
   - 👍/👎 评价每条 AI 回答（可附带原因和意见）
   - 没找到答案时点击"提交问题"，我们会尽快补充知识库
   - 知识库无相关内容时，系统会明确告知并引导转人工，不生成无依据的答复
   - 🌙 支持深色模式（右上角一键切换，记住您的选择），手机/PAD 可使用（响应式布局）
